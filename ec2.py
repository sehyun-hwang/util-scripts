...
def choose_instance():
    instances = sum(
        [
            x["Instances"]
            for x in ec2.describe_instances(
    ...
    descriptions = [
        (
            [": ".join(x.values()) for x in instance.get("Tags", {})],
            instance["InstanceType"],
            images[instance["ImageId"]],
        )
...
def init_instance(user, ip):
    ...
    client.connect(ip, username=user, key_filename=str(KEY_FILE))
    ...
    if len(stdout):
        ...
        print("Mounting EFS")
        stdout, stderr = client.exec_command(f"sudo mount -t efs {get_efs()} /mnt")[1:]
